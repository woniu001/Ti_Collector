        ...
        if '#' in eachline:
            ...
            if ipgroup != 'None':
                for ip in ipgroup:
                    if len(ip) < 7:
                        continue
                    cursor.execute("REPLACE INTO ip_table(ip,update_time,source,stamp) VALUES('%s','%s','%s','%s')" % (
                        ip, update_time, str(source), stamp))
                continue
            try:
                eachline_parameter = eachline.split()[1]
                domain = eachline_parameter.strip('\n').strip()
                cursor.execute(
                    "REPLACE INTO domain_table(domain,update_time,source,stamp) VALUES('%s','%s','%s','%s')" % (
                        domain, update_time, source, stamp))
            except:
                try:
...
def download_url(source, stamp, url):
    ...
    for eachline in fo:
        ...
        if '#' in eachline:
            ...
            cursor.execute(
                "REPLACE INTO url_table(url_index,url,update_time,source,stamp) VALUES('%s','%s','%s','%s','%s')" % (
                    url[0:50], url, update_time, str(source), stamp))
    db.commit()
    db.close()
    fo.close()
